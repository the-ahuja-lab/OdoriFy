...
def user_predict(model, x_input_smile, x_input_seq, count, filename):
    ...
    ax.figure.savefig('{}_{}_SmileInterpretability.pdf'.format(filename,count))
...
#     molecule structure interpretability
    ...
    fp = open('{}_{}_mol.svg'.format(filename,count), "w")
    print(svg, file=fp)
    fp.close()
...
#     Sequence Interpretability
    ...
    ax.figure.savefig('{}_{}_SequenceInterpretability.pdf'.format(filename,count))
    return z
...
for i in range(number_of_rows):
    ...
    z = user_predict(loaded_model, user_smile, user_seq, i + 1, filename)
